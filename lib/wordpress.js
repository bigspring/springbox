...
var WpInstall = function(wp, options, config) {
	return [
		...
		function(callback) {
			...
			wp.config.create(config.db, function(err, res) {
				callback(err, res);
			});
		},
		function(callback) {
			if ('127.0.0.1' === config.db.dbhost) {
		...
		// function(callback) {
		// 	log('Removing default themes...'.green.bgBlue);
		// 	var byeThemes = ['twentyfourteen', 'twentyfifteen'];
			...
			prompt.get({
				...
				function (err, res) {
					...
					if (res.wp_export_db === 'n') {
						...
						log('Exporting database...'.green.bgBlue);
						wp.db.export(null, function(err, res) {
							callback(err, res);
						});
					}
				}
...
module.exports = function(config, options) {
	config.db = {
		dbname: config.db_name || 'sbx_' + options.name.short,
		dbuser: config.db_username,
		dbpass: config.db_password,
		dbprefix: config.wp_prefix + '_',
		dbhost: config.db_address || '127.0.0.1'
	};
